...
(function(){
	...
	ItemGroup.prototype.shrink = function() {
		...
		this.child.forEach((c) => table_body.appendChild(c.el));
		this.child.forEach((c) => c.el.classList.add('hidden_row'));
	};
	...
	ItemGroup.prototype.expand = function() {
		...
		if(this.row) {
			...
			this.child.reverse();
			this.child.forEach((c) => c.el.classList.remove('hidden_row'));
		}
	};
